...
export class FilterService {
  //countryChanged: EventEmitter<Object> = new EventEmitter();
  private filterText='';
  ...
  countries = ['Afghanistan', 'Albania', 'Algeria', 'Andorra', 'Angola',
    ...
    'Jamaica', 'Japan', 'Jersey', 'Jordan', 'Kazakhstan', 'Kenya', 'Kuwait', 'Kyrgyz Republic', 'Laos', 'Latvia'
  ...
  renderFilteredCountries(): string[]{
    return this.countries.filter((name)=>name.toLowerCase().includes(this.filterText.toLowerCase()));
  }
  ...
  // filterCountries(filterVal) {
  //   this.filterText = filterVal;
  // }

  // giveFullList() {
  //   return this.countries.filter((country)=>country.toLowerCase().includes(this.filterText.toLowerCase()));
  // }
